...
router.post('/signup', async (req, res) => {
  ...
  if (await users.findOne({email: req.body.email})) {
    ...
    urlShortener.short('https://www.collegehub.co.zw/confirm-signup/' + key, function(err, shortURL){
      ...
      sgMail.send(msg).then(message => console.log({"success": message})).catch(error => console.log({"error": error}))
      client.messages.create(sms).then(message => console.log(message.sid)).catch(error => console.log(error))
